...
namespace velodyne_rawdata
{
  ...
  void RawData::unpack(const velodyne_msgs::VelodynePacket &pkt,
                       velodyne_msgs::VelodynePointCloudByScan &pc)
  {
        ...
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          ...
          if ((azimuth_corrected >= config_.min_angle
            ...
            if (pointInRange(distance)) {
              ...
              // append this point to the cloud
              velodyne_msgs::VelodynePoint point;
              point.time = packet_start_time + (block*VLP16_BLOCK_TDURATION) +
              ...
              point.y = y_coord;
              point.z = z_coord;
              point.azimuth = static_cast<float>(azimuth_corrected) * M_PI / 18000.0f;
              point.altitude = corrections.vert_correction;
              point.distance = distance;
              point.intensity = (uint8_t) intensity;

              pc.scans[point.ring].points.push_back(point);
            }
          }
        }
      }
